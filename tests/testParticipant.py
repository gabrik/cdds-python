...
from cdds.participant import Participant
from cdds.runtime import Runtime
...
class ParticipantBasicTest (unittest.TestCase):
    ...
    def test_find_topic(self):
        ...
        self.assertTrue(topic_to_test is not None,
                        "Found topic should not be none")
        # self.assertEqual(self.topic.topic, foundTopic, "Find topic failed")
        self.assertEqual(self.topic.name, found_topic_name,
                         "Find using name failed")
