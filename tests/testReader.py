...
class DataReaderTest(unittest.TestCase):
    ...
    def test_read_mask(self):
        ...
        for i in range(33, 36):
            ...
            print("written sample {0} ".format(newMsg))
            rc = self.writer.write(newMsg)
            self.assertEqual(rc, 0, "An error occurred while write operation")
        time.sleep(5)
                ...
                if samples is not None:
                    ...
                    for s in list(filter(lambda x: x[1].valid_data, samples_as_list)):
                        if s[0] is not None and s[1].valid_data:
                            ...
                            print('val_.userID {}'.format(val_.userID))
                            print('val_.message {}'.format(val_.message))
                            self.assertEqual(val_.userID, 33, "unexpected message read")
                ...
                if samples is not None:
                    ...
                    for s in list(filter(lambda x: x[1].valid_data, samples_as_list)):
                        if s[0] is not None and s[1].valid_data:
                            ...
                            self.assertEqual(s[1].instance_handle, inst_hdl, "Instance handle of read sample is not the same as the one found in lookup_instance")
                else:
    ...
    def test_take_mask(self):
        ...
        for i in range(113, 116):
            ...
            print("written sample {0} ".format(newMsg))
            rc = self.writer.write(newMsg)
            self.assertEqual(rc, 0, "An error occurred during write operation")
        time.sleep(5)
