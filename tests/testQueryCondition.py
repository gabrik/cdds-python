...
class QueryConditionTest (unittest.TestCase):
    def setUp(self):
        ...
        self.rt.funclib.filter_mod2.restype = c_bool
        self.rt.funclib.filter_mod2.argtypes = [c_void_p]
    ...
    def test_init_QueryCondition(self):
        ...
        self.assertIsNotNone(cond, "QueryCondition creation faild")
    ...
    def test_read(self):
                ...
                if samples is not None:
                    ...
                    for s in list(filter( lambda x: x[1].valid_data, samples_as_list)):
                        if s[0] is not None and s[1].valid_data:
                            ...
                            self.assertTrue(val_.userID in range(0, 10), "Read message has an invalid key {0}".format(val_.userID))
            except:
                print ("Invalid data read")
                

        self.assertEqual(cnt_samples, 10, "read samples are not as expected")
        ...
        msg = HelloWorldData_Msg(userID = 6, message = "new message ")
        print("Writer >> Begin writeing data")
                ...
                if samples is not None:
                    ...
                    for s in list(filter( lambda x: x[1].valid_data, samples_as_list)):
                        if s[0] is not None and s[1].valid_data:
                            ...
                            self.assertTrue(val_.userID is 6, "Read message has an invalid key {0}".format(val_.userID))
            except:
                print ("Invalid data read")
                raise Exception("Unexpected error:", sys.exc_info()[0])
    ...
    def test_read_n (self):
        ...
        while(not sample_read) : 
            ...
            mask2 = DDS_NOT_READ_SAMPLE_STATE | DDS_NEW_VIEW_STATE | DDS_ALIVE_INSTANCE_STATE
